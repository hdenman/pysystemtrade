...
Paper must keep its own Mongo DB and its own FerretDB SQLite directory because Mongo stores order stacks, historic orders, roll state, process control, and contract metadata. Do not share Mongo collections between paper and live. If both instances run simultaneously, run separate FerretDB processes with separate working directories and ports.
...
export MONGO_DATA=/srv/pysystemtrade/mongo-data/paper
export MONGO_DATA_U=/srv/pysystemtrade/mongo-data/paper
export PARQUET_DATA=/srv/pysystemtrade/paper/parquet
export PYSYS_PRIVATE_CONFIG_DIR=/srv/pysystemtrade/paper/pysystemtrade_config
export PYTHONPATH=/srv/pysystemtrade/code/pysystemtrade:${PYTHONPATH:-}
...
export MONGO_DATA=/srv/pysystemtrade/mongo-data/live
export MONGO_DATA_U=/srv/pysystemtrade/mongo-data/live
export PARQUET_DATA=/srv/pysystemtrade/live/parquet
export PYSYS_PRIVATE_CONFIG_DIR=/srv/pysystemtrade/live/pysystemtrade_config
export PYTHONPATH=/srv/pysystemtrade/code/pysystemtrade:${PYTHONPATH:-}
cd "$PYSYSTEMTRADE_HOME"
```
...
- Clean start: create empty `production_live` DB in `/srv/pysystemtrade/mongo-data/live` and initialise capital/process state through normal pysystemtrade setup.
- Optional one-way reference copy: copy only proven reference collections, such as `futures_contracts` or `spread_costs`, from paper/live into the other DB as an explicit maintenance operation.

Boring/safe recommendation: clean start. Contract metadata can be generated independently by `update_sampled_contracts` in live. Do not share Mongo files or collections between paper and live at runtime.
...
Recommended solution: run `update_sampled_contracts` directly from a paper-owned wrapper. The module's default CLI behavior should be non-interactive: no argument means update all instruments; pass `--interactive` only for manual prompt-driven operation.

Create a paper-owned shell wrapper outside the shared code checkout:

```bash
# /srv/pysystemtrade/paper/bin/update-sampled-contracts
#!/usr/bin/env bash
set -euo pipefail
source /srv/pysystemtrade/paper/.profile
process=update_sampled_contracts
stamp=$(date +%Y-%d-%m)
...
exec /srv/pysystemtrade/code/pysystemtrade/.devenv/state/venv/bin/python -m sysproduction.update_sampled_contracts
```
...
sudo chmod +x /srv/pysystemtrade/paper/bin/update-sampled-contracts
...
Use separate FerretDB processes, ports, working directories, SQLite directories, and DB names:

```text
paper:
  user: pst-paper
  working directory: /srv/pysystemtrade/paper
  dbpath: /srv/pysystemtrade/mongo-data/paper
  mongo_host: 127.0.0.1
  mongo_port: 27017
  mongo_db: production_paper    # actual DB: production_paper_<universe>

live:
  user: pst-live
  working directory: /srv/pysystemtrade/live
  dbpath: /srv/pysystemtrade/mongo-data/live
  mongo_host: 127.0.0.1
  mongo_port: 27018
  mongo_db: production_live     # actual DB: production_live_<universe>
```

The separate working directories keep FerretDB process state files separate:

```text
/srv/pysystemtrade/paper/state.json
/srv/pysystemtrade/live/state.json
```

Do not share Mongo DBs, SQLite files, or runtime collections between paper and live.

Mongo-backed state includes order stacks, historic orders, roll state, process control, futures contract metadata, and stored spread costs. Some collections may look reusable, but `pysystemtrade` expects one Mongo DB per process and does not route individual collections to separate stores.

If you later want to share only reference-like Mongo collections, do it as an explicit one-way copy job after proving the collection is safe. Never get sharing accidentally by using the same `mongo_db`, same FerretDB dbpath, or same FerretDB process.

Default policy:

- Share Parquet market data.
- Do not share Mongo.
- Rebuild contract metadata independently with `update_sampled_contracts`, or copy selected reference collections explicitly.
...
- `pst-paper` can import/run pysystemtrade from `/srv/pysystemtrade/code/pysystemtrade`.
...
- `pst-paper` Mongo DB is `production_paper...`, served by paper's FerretDB instance, not live.
...
- `pst-live` Mongo DB is `production_live...`, served by live's FerretDB instance, not paper.
...
- Backups cover both Mongo SQLite directories, both local Parquet trees, and the shared market-data tree.
...
7. Copy config skeleton to `pst-live`; both users use shared code.
8. Configure `pst-live` as live account with separate Mongo DB, separate FerretDB instance/port, and live Gateway.
